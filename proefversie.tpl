...
<style>
  * { box-sizing: border-box; -webkit-tap-highlight-color: transparent; }
  ...
  #enemySvg svg { position: absolute; inset: 0; width: 100%; height: 100%; overflow: visible; }
  /* De gooey-canvas is groter dan het monster zodat de gloed erbuiten past. */
  #enemyGoo { position: absolute; left: 50%; top: 50%; width: 170%; height: 170%;
              transform: translate(-50%, -50%); pointer-events: none; }
  #enemySvg.goo svg { display: none; }
  ...
  .lbLevel { flex: none; font-size: 19px; font-weight: 900; font-variant-numeric: tabular-nums; }
  ...
  <div class="middle">
    <div class="arenaCol">
      ...
      <div id="orbwrap">
        ...
        <div id="bossRing"></div>
        <div id="enemySvg">
          <canvas id="enemyGoo"></canvas>
          <svg viewBox="0 0 100 100"><path id="enemyPath"/></svg>
        </div>
      </div>
      <div class="hpwrap">
...
const orbMix = (c, f) => {
  const t = [0.95, 0.15, 0.22];
  return c.map((v, i) => t[i] + (v - t[i]) * Math.max(0, Math.min(1, f)));
};
const orbCss = (c, f, a=1) => rgbCss(orbMix(c, f), a);

/* ---------------------------------------------------------------
   Vloeibaar monster (naar de "Liquid Gooey Orb"-shader van Tamino
   Martinius). Het silhouet van de vijand wordt een traag golvende
   vorm met wervelend vloeibaar binnenwerk, dunne lichte sliertjes,
   een glazige rand en een gloed die buiten het silhouet blijft —
   alles in de kleur die het monster op dat moment heeft, dus ook
   het verkleuren naar rood bij schade doet gewoon mee.
   Zonder WebGL2 blijft de gewone SVG-weergave staan.
---------------------------------------------------------------- */
const GOO = (() => {
  ...
  uniform vec3 u_licht;   // lichte variant voor sliertjes en gloedaccenten
  ...
  vec2 golf(vec2 p, float t){
    return vec2(
      sin(p.y * 7.0 + t)        + 0.6 * sin((p.x + p.y) * 11.0 - t * 0.8),
      sin(p.x * 8.0 - t * 0.9 + 1.7) + 0.6 * sin((p.y - p.x) * 10.0 + t * 0.7));
  }
  ...
  // domain-warped veld -> stromende sliertjes, net als in de pen
  float liquid(vec3 p){
    float t = u_tijd * 0.5;
    p *= 2.2;
    p.xy *= rot(t * 0.15);
  ...
  void main(){
    ...
    vec2 w = uv + golf(p * 2.0, t * 0.9) * 0.011;
    float d = sdf(w);

    // pseudo-normaal uit het afstandsveld, voor rand en glinsters
    ...
    for (int k = 0; k < 6; k++) {
      float raw = liquid(rp);
      float dens = smoothstep(0.35, 0.75, raw);
    ...
    // glazige rand + twee glinsters, zoals de fresnel en speculars in de pen
    float fres = pow(clamp(1.0 - diepte / 0.055, 0.0, 1.0), 3.0);
    vec3 rim = mix(u_basis, u_licht, 0.5 + 0.5 * (n.x * 0.7 + n.y * 0.45));
    vec3 Ein = inner * (1.0 - fres * 0.6) + rim * fres * 1.1;
    vec3 h1 = normalize(vec3(0.45, 0.65, 1.0));
    vec3 h2 = normalize(vec3(-0.55, 0.20, 1.0));
    Ein += vec3(1.0) * pow(max(dot(n, h1), 0.0), 140.0) * 1.3;
    Ein += vec3(0.9, 0.95, 1.0) * pow(max(dot(n, h2), 0.0), 60.0) * 0.5;

    // gloed alleen buiten het silhouet, kleur draait langzaam rond
    float gloed = exp(-max(d, 0.0) * 22.0);
    float ang = atan(p.y, p.x);
    vec3 gc = mix(u_basis, u_licht, 0.5 + 0.5 * sin(ang * 3.0 + t * 0.5));
    vec3 Eg = (gc * gloed * 1.4 + vec3(1.0) * pow(gloed, 3.0) * 0.5) * 0.75 * (1.0 - aa);
    float cov = clamp(max(Eg.r, max(Eg.g, Eg.b)), 0.0, 1.0);
  ...
  function compileer(soort, bron) {
    const sh = gl.createShader(soort);
    gl.shaderSource(sh, bron);
    gl.compileShader(sh);
  ...
  const U = {
    tijd: gl.getUniformLocation(programma, 'u_tijd'),
    basis: gl.getUniformLocation(programma, 'u_basis'),
    licht: gl.getUniformLocation(programma, 'u_licht'),
  };
  ...
  function chamfer(v, N) {
    for (let y = 0; y < N; y++) for (let x = 0; x < N; x++) {
      ...
      if (x > 0) m = Math.min(m, v[i - 1] + 3);
      if (y > 0) {
        m = Math.min(m, v[i - N] + 3);
        if (x > 0) m = Math.min(m, v[i - N - 1] + 4);
        if (x < N - 1) m = Math.min(m, v[i - N + 1] + 4);
      }
      v[i] = m;
    }
    for (let y = N - 1; y >= 0; y--) for (let x = N - 1; x >= 0; x--) {
      const i = y * N + x; let m = v[i];
      if (x < N - 1) m = Math.min(m, v[i + 1] + 3);
      if (y < N - 1) {
        m = Math.min(m, v[i + N] + 3);
        if (x < N - 1) m = Math.min(m, v[i + N + 1] + 4);
        if (x > 0) m = Math.min(m, v[i + N - 1] + 4);
  ...
  const sdfTexturen = new Map();
  function sdfTextuur(pad) {
    if (sdfTexturen.has(pad)) return sdfTexturen.get(pad);
    const N = SDF_N;
    const vlak = document.createElement('canvas');
    vlak.width = vlak.height = N;
    const ctx = vlak.getContext('2d', { willReadFrequently: true });
    ctx.translate(N / 2, N / 2);
    ctx.scale(N / (100 * SDF_RUIMTE), N / (100 * SDF_RUIMTE));
    ctx.translate(-50, -50);
    ctx.fillStyle = '#fff';
    ctx.fill(new Path2D(pad));
    const alfa = ctx.getImageData(0, 0, N, N).data;
    const INF = 1e9, buiten = new Float64Array(N * N), binnen = new Float64Array(N * N);
    for (let i = 0; i < N * N; i++) {
      const in_ = alfa[i * 4 + 3] > 127;
      buiten[i] = in_ ? 0 : INF;   // afstand tot het silhouet
      binnen[i] = in_ ? INF : 0;   // afstand tot de buitenwereld
    }
    chamfer(buiten, N);
    chamfer(binnen, N);
    const data = new Uint8Array(N * N);
    for (let i = 0; i < N * N; i++) {
      const d = (buiten[i] - binnen[i]) / 3; // pixels, positief = buiten
      data[i] = Math.max(0, Math.min(255, Math.round(127.5 + d * (127.5 / 40))));
    }
    ...
    sdfTexturen.set(pad, tex);
    return tex;
  }

  const DPR = Math.min(window.devicePixelRatio || 1, 1.5);
  const RUSTIG = matchMedia('(prefers-reduced-motion: reduce)').matches;
  let basis = [0.5, 1, 0.3], licht = basis, huidig = null;

  function zet(icoon, rgb, maat) {
    basis = rgb;
    licht = rgb.map(v => v + (1 - v) * 0.6);
    const px = Math.round(maat * SDF_RUIMTE * DPR);
    if (canvas.width !== px) canvas.width = canvas.height = px;
    if (huidig !== icoon) {
      huidig = icoon;
      gl.bindTexture(gl.TEXTURE_2D, sdfTextuur(icoon));
    }
  }

  function stap(nu) {
    requestAnimationFrame(stap);
    if (huidig === null || document.hidden || !canvas.width) return;
    if (canvas.offsetParent === null) return; // scherm staat niet in beeld
    gl.viewport(0, 0, canvas.width, canvas.height);
    gl.uniform1f(U.tijd, RUSTIG ? 1.7 : nu * 0.001);
    gl.uniform3fv(U.basis, basis);
    gl.uniform3fv(U.licht, licht);
...
function render() {
  ...
  svg.classList.toggle('goo', GOO.actief);
  if (GOO.actief) {
    // De shader tekent zijn eigen gloed; de zware drop-shadows mogen uit.
    svg.style.filter = 'none';
    GOO.zet(arena.icon, orbMix(c, f), size);
  } else {
    svg.style.filter = `drop-shadow(0 0 14px ${orbCss(c, f, .9)}) drop-shadow(0 0 40px ${orbCss(c, f, .5)})`;
  }
  // De zachte gloeicirkel hoort bij de SVG-weergave; de shader gloeit zelf.
  $('glow').style.display = GOO.actief ? 'none' : 'block';
